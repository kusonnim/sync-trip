...
The production-configurable FastAPI service provides place-information endpoints, deterministic Track 1 optimization, and bounded Track 2 live-route refinement.
...
Accepts the snake_case payload generated by `frontend/src/lib/api.js#buildOptimizeBody` and returns `min_time` and `min_cost` route options. In frontend backend mode, set `VITE_API_BASE` to this service's origin.
